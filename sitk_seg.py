...
def main_fun(data_name):
    ...
    indicess2 = np.where(llmask == 0)
    dilate_img_array = sitk.GetArrayFromImage(dilate_img) 
    dilate_img_array[indicess2] = 0
    dilate_img_filtered = sitk.GetImageFromArray(dilate_img_array)
    dilate_img_filtered.CopyInformation(dilate_img)

    ###########################################################################
    print "anding image"
    # invert to get any other tissues back
    invert_img = sitk.InvertIntensity(dilate_img_filtered, maximum=1.0)
    ...
    ###########################################################################
    # remove both lung and body
    indicess2 = np.where(llmask > 0)
    and_image_array = sitk.GetArrayFromImage(processed_img)
    and_image_array[indicess2] = 0
    and_image = sitk.GetImageFromArray(and_image_array)
    and_image.CopyInformation(invert_img)
    print "operating final labelmapping"
    # sitk.Show(and_image)
    
    ###########################################################################
    lmap = sitk.BinaryImageToLabelMap(and_image, fullyConnected=True)
    limg = sitk.LabelMapToLabel(lmap)
